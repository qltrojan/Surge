...
let smsCode = ($.isNode() ? process.env.Common_smsCode : $.getdata("Common_smsCode")) || "";
let password = ($.isNode() ? process.env.Common_Password : $.getdata("Common_Password")) || "";
let Utils = undefined;
let signature_key = ''
let tenantId = ''
let ua = ''
...
let notice = ''
!(async () => {
    if (phone == "" || appType == "" || smsCode == "" || password == "") {
...
async function main() {
    ...
    console.log("验证验证码")
    let verify = await passportGet(`/web/security/check_security_code?client_id=${clientId}&phone_number=${phone}&security_code=${smsCode}`);
    if(verify.code == 0) {
        console.log("验证码验证成功")
        console.log("重置密码")
        let reset = await passportPost('/web/oauth/reset_password',`client_id=${clientId}&new_password=${password}&phone_number=${phone}&security_code=${smsCode}`)
        if (reset.code == 0) {
            console.log("重置密码成功")
        } else if (reset.code == 100001) {
            console.log(reset.message)
            console.log("开始注册")
            console.log("获取code")
            let verify = await passportPost('/web/oauth/security_code_auth',`client_id=${clientId}&phone_number=${phone}&security_code=${smsCode}`);
            ...
            console.log(`注册结果：${login.message}`)
        } else {
            console.log(reset.message)
        }
    } else {
...
function getBody(url,body) {
    ...
    if (!body) {
        ...
        body = `client_id=${clientId}&new_password=${encodeURIComponent(password)}&phone_number=${phone}&security_code=${smsCode}`
        CryptoJS = Utils.createCryptoJS();
        const hash = CryptoJS.HmacSHA256(str, signature_key);
        let signature = CryptoJS.enc.Hex.stringify(hash);
