...
async function main() {
    ...
    for (const item of arr) {
        ...
        console.log(`获取userId：${loginPwd.data.userId}`)
        userId = loginPwd.data.userId;
        console.log(`获取name：${loginPwd.data.nickname}`)
        ...
        for (const news of newsList.data.content) {
            if (news.title.includes('转盘')) {
                console.log(`抽奖活动：${news.title}`);
                newsId = news.id;
                break
            }
        }
        ...
        if (!newsId) {
            ...
            console.log(`拥有${lottery.element.freeLimit}次抽奖`)
        ...
        if (!readFinish || !likeFinish || !shareFinish) {
            let articleList = await commonGet('/yongpai-news/api/news/list?channelId=2&currentPage=1&timestamp=0')
            let count = 30
