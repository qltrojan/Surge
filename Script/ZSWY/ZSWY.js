...
let commonUa = ''
let deviceId = ''
let jinhuaAppId = 'vKmnytOp9GrPa7kLbWTx'
...
async function main() {
    ...
    for (const item of arr) {
        ...
        console.log('阅读抽奖')
        console.log("获取id")
        let articleList = await commonGet('/api/article/channel_list?channel_id=657a5b3d79f6be03b8fd7fb1&isDiFangHao=false&is_new=true&list_count=0&size=20')
        let url = articleList.data.article_list[0].column_news_list[0].url;
        ...
        for (const item of jinhuaDetail.data.levels) {
            ...
            for (const task of level.data.tasks) {
                ...
                let detail = await commonGet(`/api/article/detail?id=${result.id}`)
                console.log(`文章：${detail.data.article.list_title}`)
                let like = await commonPost('/api/favorite/like',`action=true&id=${result.id}`)
                console.log(`点赞：${like.message}`)
                let read = await commonGet(`/api/article/read_time?channel_article_id=${result.id}&read_time=5938`)
        ...
        if (!readFinish || !likeFinish || !shareFinish) {
            let articleList = await commonGet('/api/article/channel_list?channel_id=6423f0a579f6be58d033d816&isDiFangHao=false&is_new=true&list_count=0&size=20')
